...
GUESS_TIME_LIMIT = 30000   # 30 seconds
...
while gameRunning:
    ...
    # ── WAIT FOR GUESS (MAX 30s) ─────────────────────────
    guess = None
    startTime = time.ticks_ms()

    while guess is None:

        if time.ticks_diff(time.ticks_ms(), startTime) > GUESS_TIME_LIMIT:
            print("No guess received - FAIL")
            btWrite("FAIL")
