...
#   BLE      — MIT App Inventor sends "R,G,B\n", receives PASS/FAIL/etc.
#
# Flow:
#   Power on → wait for BLE → rainbow snake + idle blinking
#   Button press → game starts (5 rounds, threshold tightens each round)
#   Each round: eye closes 3 s → NeoPixel flashes target colour 3x →
#               eye closes, app sends guess → evaluate → green/red flash
#   Game over → back to idle (rainbow snake + blinking)
...
# ─────────────────────────────────────────────────────────────────────────
SERVO1_OPEN_ANGLE  = 140
SERVO1_CLOSE_ANGLE = 20
...
# ─────────────────────────────────────────────────────────────────────────
# Colour Palette
# Vibrant hues but at reduced brightness (shades) so real-world photos
# at lower resolution still match accurately.
# Rule: at least one channel at full saturation, overall brightness ~55-70%
# ─────────────────────────────────────────────────────────────────────────
PALETTE = [
    (180,   0,   0),   # deep red
...
# Original thresholds — tighter as rounds progress
...
# Rainbow snake state (global so idle loop can resume smoothly)
_snake_offset = 0

def rainbowSnakeTick(speed=40):
    # Advance the snake by one step and write to ring.
    # Call this repeatedly in a loop; does NOT block.
    global _snake_offset
    snake_len  = 6     # how many lit pixels in the snake
    tail_fade  = True  # fade the tail for a softer look

    for i in range(pixels):
        pos = (i - _snake_offset) % pixels
        if pos < snake_len:
            hue      = (_snake_offset * 8 + i * (360 // pixels)) % 360
            # brightness: full at head, fading toward tail
            if tail_fade:
...
# Runs until button is pressed. Returns when button pressed.
# ─────────────────────────────────────────────────────────────────────────
def idleLoop():
    openEyes()
...
# ─────────────────────────────────────────────────────────────────────────
# Wait for BLE then show loading rainbow until button pressed
# ─────────────────────────────────────────────────────────────────────────
...
# Wait for BLE connection — show a slow dim pulse while waiting
...
# ─────────────────────────────────────────────────────────────────────────
# Main outer loop — idle then game, repeat
# ─────────────────────────────────────────────────────────────────────────
while True:

    # ── Idle: rainbow snake + blinking until button ──────────────────────
    idleLoop()
    ...
    while gameRunning:
        ...
        # ── Close eye for 3 seconds (anticipation) ───────────────────────
        closeEyes()
        time.sleep_ms(3000)

        # ── Open and flash target colour 3 times ─────────────────────────
        openEyes()
        flash(targetRgb, FLASH_COUNT, FLASH_ON_MS, FLASH_OFF_MS)

        # ── Close eyes while player takes photo / picks colour ───────────
        allOff()
        closeEyes()
        ble.send("WAIT")
        time.sleep_ms(300)
        ...
        if dist <= threshold:
            ...
            if round_num >= MAX_ROUNDS:
                ...
                # prompt app to send NEXT when player is ready
